...
defmodule Queue do
  ...
  def handle_cast({:consumer, consumer_id, consumer_pid} = consumer, %{
        jobs: jobs,
        consumers: consumers,
        buffer: size
      }) do
  ...
  def handle_cast(
        {:producer, job, producer_id} = producer,
        %{jobs: jobs, consumers: consumers, buffer: size} = state
      ) do
  ...
  defp process_job({:job, job, producer_id} = new_job, jobs, consumers, size)
    ...
    IO.puts(
      "Stats #{stats(jobs, consumers)}: Full. Discarding job #{job} from PRODUCER #{producer_id}"
    )
  ...
  defp process_job({:job, job, producer_id} = new_job, jobs, consumers, size) do
    ...
    IO.puts(
      "Stats #{stats(jobs, consumers)}: Sending job #{job} from PRODUCER #{producer_id} to CONSUMER #{
        consumer_id
      }"
    )
